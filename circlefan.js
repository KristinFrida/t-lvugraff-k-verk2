...
window.onload = function init() {
    ...
    var label  = document.getElementById("points-val");
    if (label) label.textContent = numCirclePoints;
    
    if (slider) {
        slider.addEventListener('input', function() {
            var numPoints = Math.max(3, Math.min(50, parseInt(slider.value, 10)));
            label.textContent = numPoints;
            numCirclePoints = numPoints;
            createCirclePoints(center, radius, numPoints);
            gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
